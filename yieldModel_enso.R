...
#remove first year
dF.enso<- dF %>% filter(season!="2014/15")
...
(fm001<-lmer(HC_log~1+(1|plot/Transect)+(1|low.plot),data=dF.enso,REML=F))
(fm001b<-lmer(HC_log~1+(1|plot)+(1|low.plot),data=dF.enso,REML=F))
(fm001c<-lmer(HC_log~1+(1|low.plot)+(1|Transect),data=dF.enso,REML=F))
(fm001d<-lmer(HC_log~1+(1|low.plot),data=dF.enso,REML=F))
(fm001e<-lmer(HC_log~1+(1|Transect),data=dF.enso,REML=F))
(fm001f<-lmer(HC_log~1+(1|plot/Transect),data=dF.enso,REML=F))
(fm001h<-lmer(HC_log~1+(1|season) + (1|Transect),data=dF.enso,REML=F))
(fm001i<-lmer(HC_log~1+(1|season) ,data=dF.enso,REML=F))
(fm001g<-glm(HC_log~1,data=dF.enso))
anova(fm001,fm001b,fm001c,fm001d,fm001e,fm001f,fm001g,fm001h,fm001i) #use transect and plot as random effects

#full model of log difference in yield (2015/2014 and 2016/2014)
(gm01<-lmer(HC_log~Age.of.cocoa + Cocoa.density+Canopy.gap.dry+CN.ratio+Tot.P+K.meq+soil.moist+stress.mm+PropCPB+PropBP+Biomass+distance.cont+No.applications.yr+Tmax+maxVPD+low.plot+(1|plot),data=dF.enso,REML=F))
summary(gm01)
gm01s<-standardize(gm01)
summary(gm01s)

(gm01<-lm(HC_log~Age.of.cocoa + Cocoa.density+Canopy.gap.dry+CN.ratio+Tot.P+K.meq+soil.moist+stress.mm+PropCPB+PropBP+Biomass+distance.cont+No.applications.yr+Tmax+maxVPD+low.plot,data=dF.enso))
summary(gm01)
gm01s<-standardize(gm01)
summary(gm01s)

(gm02<-lmer(HC_log~Age.of.cocoa + Cocoa.density+Canopy.gap.dry+CN.ratio+Tot.P+K.meq+stress.mm+PropCPB+PropBP+distance.cont+Tmax+maxVPD+(1|plot),data=dF.enso,REML=F))
summary(gm02)
gm02s<-standardize(gm02)
summary(gm02s)
...
diagnos.enso <- data.frame(Resid = resid(gm02s, type = "pearson"), Fitted = fitted(gm02s),Variable = dF.enso$plot )
pdf(paste0(getwd(),"/Analysis/ElNino/Plot.logyld_ResidualvFittedValues_all.pdf"),width=8,height=8)
xyplot(Resid ~ Fitted, data = diagnos.enso)
dev.off()

pdf(paste0(getwd(),"/Analysis/ElNino/Plot.logyld_qqplotResiduals_all.pdf"),width=8,height=8)
